...
## M6 durable reference checks

With the isolated local Postgres migration applied: `TEST_DATABASE_URL=postgresql://127.0.0.1:55432/astraworld npm run test:durable`. The fault suite requires loopback Postgres and owns Redis port 6403, TCP proxy 55434 and gateway 3231; it drops actual database connections and SIGKILLs its child runner. The network recovery suite owns Redis 6402 and gateways 3221/3222. Tests skip DB checks when `TEST_DATABASE_URL` is absent; do not mistake that for durability verification.

`tests/e2e/recovery.spec.ts` exercises real gathering, reload, recovery-file download and import into a fresh context. Use `BASE_URL`, optional private `HOSTED_ACCESS_FILE`, and `RECOVERY_EVIDENCE_PREFIX` for hosted verification without printing credentials. `scripts/verify-database-restore.mjs` accepts `SOURCE_DATABASE_URL` and a distinct empty loopback `TARGET_DATABASE_URL`; it restores only isolated `test:` namespace records and removes its private temporary backup. [M6 results](milestones/M6_DURABLE_RELEASE.md) distinguish application recovery from provider PITR.
