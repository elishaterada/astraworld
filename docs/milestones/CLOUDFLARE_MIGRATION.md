...
- Production Worker version `52915643-1151-48c9-a878-ef12a3e45083` deployed with the custom hostname. Public DNS/HTTPS and email activation/delivery checks are pending at this checkpoint. Email activation before cutover correctly returned “Active zone required.” Do not treat prepared routing as delivered email.
...
1. Keep Vercel's immutable production deployment and the existing Redis/Neon integrations. The database namespace is preserved, so compatible gateways can recover committed state. Do not restore the backup over newer acknowledged progress during an ordinary hosting rollback.
