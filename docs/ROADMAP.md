...
**M0 is complete locally (G0/R0/R1 and ten-minute performance PASS)**; see [its completion evidence](milestones/M0_COMPLETION.md). M1 is [complete within the recorded test envelope](milestones/M1_COMPLETION.md), including deployed lifecycle/overlap, real TCP loss and the eight-player soak; M2 gathering and inventory is [complete with local and hosted evidence](milestones/M2_GATHERING_INVENTORY.md); M3–M9 are **not started**. Milestone names below normalize the different numbering used in the conversation; use these IDs in future tasks.
