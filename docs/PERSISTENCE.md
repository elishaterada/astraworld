...
## M7 crafting and station state

The existing versioned JSONB aggregate adds optional `gathering.benches` (maximum two, definition-validated positions and member owners). Absent data loads as no placements, so existing M6 saves need no table migration or namespace reset. Craft/place receipts use the existing durable gather journal with the action included in its request hash. Costs, tool creation and station ownership commit atomically. Durable validation rejects disappearance or ownership/position regression of an existing bench. No dismantling or refunds are introduced. [M7 evidence](milestones/M7_FIRST_CRAFTING.md).
