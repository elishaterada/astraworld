...
it.skipIf(!process.env.TEST_DATABASE_URL)(
  "M6 commits gathering, taming and gates before publication and restores after all cache and runner state is lost",
  async () => {
    const redis = spawn(
      "redis-server",
      [
    ...
    const store = new Store(
      ...
      1800,
      8,
      new DurableStore(process.env.TEST_DATABASE_URL!, `test:m6-durable`),
    );
    const connections: {
      ws: WebSocket;
    ...
    try {
      ...
      const options = {
        ...
        redisUrl: "redis://127.0.0.1:6402",
        prefix: store.prefix,
        origins: ["http://test"],
      };
      a = await createRealtimeGateway({ ...options, owner: "a" });
      b = await createRealtimeGateway({ ...options, owner: "b" });
      await new Promise<void>((r) => a!.server.listen(3221, "127.0.0.1", r));
      await new Promise<void>((r) => b!.server.listen(3222, "127.0.0.1", r));
      ...
      for (const gateway of [a, b]) {
        const original = gateway.store.durable!.commit.bind(
          gateway.store.durable,
        );
        gateway.store.durable!.commit = async (...args) => {
          const revision = await original(...args);
          const state =
            args[3] as import("../apps/game-server/durable").DurableState;
          if (
            !lostCommitReply &&
            state.gathering.players[first.playerId]?.receipt?.result ===
              "gathered"
          ) {
            lostCommitReply = true;
            // Real Postgres commit has finished, but no Redis checkpoint/reply has been published.
            const cached = await store.read(first.worldId);
            expect(
              (cached.checkpoint as typeof state)?.gathering.players[
                first.playerId
              ]?.receipt?.result,
            ).not.toBe("gathered");
            throw Error("Injected lost COMMIT reply");
          }
          return revision;
        };
      }
      const c1 = await client(3221, first),
        c2 = await client(3222, second);
      ...
      const replacement = await client(3222, first);
      ...
      const resumed = await client(3221, first);
      ...
      const c3 = await client(3222, newcomer);
      expect(last(c3).gate!.open).toBe(true);
      expect(last(c3).gate!.openedTick).toBe(openedTick);
      expect(lostCommitReply).toBe(true);
      const committed = await store.durable!.load(first.worldId);
      expect(committed!.state!.taming!.gate.open).toBe(true);
      expect(
        committed!.state!.gathering.players[first.playerId].inventory,
      ).toEqual(last(resumed).progress!.inventory);
      ...
      a = await createRealtimeGateway({ ...options, owner: "recovered" });
      await new Promise<void>((r) => a!.server.listen(3221, "127.0.0.1", r));
      const recovered = await client(3221, first);
      expect(last(recovered).gate!.openedTick).toBe(openedTick);
      expect(last(recovered).progress!.inventory).toEqual(
        committed!.state!.gathering.players[first.playerId].inventory,
      );
      expect(
        last(recovered).moss!.filter((m) => m.owner === first.playerId),
      ).toHaveLength(1);
      recovered.companion(gate.id, 6, "dissolve");
      await sleep(300);
      expect(last(recovered).gate!.openedTick).toBe(openedTick);
      expect(await store.authenticate(first.worldId, "wrong")).toBeNull();
      for (let i = 3; i < 8; i++) await store.join(`Member ${i}`, first.invite);
      await expect(store.join("Ninth", first.invite)).rejects.toThrow("full");
    } finally {
      for (const c of connections) {
        clearInterval(c.timer);
        c.ws.terminate();
