...
it("M4 contested feeding and ownership survive replay, generation replacement and owner turnover", async () => {
  const redis = spawn(
    "redis-server",
    [
      "--bind",
  ...
  try {
    ...
    await new Promise<void>((r) => a!.server.listen(3201, "127.0.0.1", r));
    await new Promise<void>((r) => b!.server.listen(3202, "127.0.0.1", r));
    ...
    const c1 = await client(3201, first),
      c2 = await client(3202, second);
    const last = (c: typeof c1) => c.snapshots.at(-1)!;
    c1.gather(`resource:${GENERATION_VERSION}:${CONTENT_VERSION}:${first.seed}:65:63`);
    ...
    c2.gather(`resource:${GENERATION_VERSION}:${CONTENT_VERSION}:${first.seed}:63:65`);
    ...
    const replacement = await client(3202, first);
